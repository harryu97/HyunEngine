...
RigidBody::RigidBody(BodyType type,
  ...
  btRigidBody::btRigidBodyConstructionInfo info(
      (m_type == BodyType::Dynamic) ? btScalar(mass) : btScalar(0), motionState,
      m_collider->GetShape(), intertia);
...
// only valid before added to the world
...
void RigidBody::SetRotation(const glm::quat& rot) {
  ...
  auto& tr = m_body->getWorldTransform();
  tr.setRotation(btQuaternion(btScalar(rot.x), btScalar(rot.y), btScalar(rot.z),
                              btScalar(rot.w)));
  if (m_body->getMotionState()) {
    m_body->getMotionState()->setWorldTransform(tr);
  }
...
void RigidBody::ApplyImpulse(const glm::vec3& impulse) {
  ...
  m_body->applyCentralImpulse(
      btVector3(btScalar(impulse.x), btScalar(impulse.y), btScalar(impulse.z)));
}
